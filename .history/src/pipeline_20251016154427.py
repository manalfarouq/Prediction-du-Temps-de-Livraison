...
def encode_categorical(dataframe_name):
    ...
    dataframe_encoded = dataframe_encoded.drop(columns['Vehicle_Type','Time_of_Day','Preparation_Time_min','Courier_Experience_yrs','Delivery_Time_min'],axis = 1)
    dataframe_encoded = pd.get_dummies(dataframe_encoded, drop_first=False)
    
    # pour changer seulement les True et false en 0 et 1
    bool_cols = dataframe_encoded.select_dtypes(include=['bool'])
    if :
